 ...
 *
 */
function modCentralMaps(x) {

	var self = this;

	self.name = "CentralMaps";
	...
	self.str = {
		generate: "#modCentralMaps_generate",
	};


	self.load = function() {

		console.log("module loaded: CentralMaps",CENTRALGRAPHS_RESP());

/*
		$(self.config.id_html).html("<p>Hello! I'm a CentralMaps module");
*/
		handlers.insert(self, CENTRALGRAPHS_RESP(), self.handle_resp);

		$(function() {
			$( "#modCentralMaps_datepicker_begin" ).datetimepicker({
				showOn: "button",
			...
			$( "#modCentralMaps_datepicker_end" ).datetimepicker({
				showOn: "button",
				buttonImage: "/images/calendar.gif",
				buttonImageOnly: true,
				dateFormat: 'yy-mm-dd',
	...
	self.handle_resp = function(self, data) {
		...
		console.log("centralmaps handle_resp");
...
*/
		...
		for(var v in self.config_client.maps) {
			console.log("WTF");
			self.config_client.maps[v].central(data);
		}
	...
	self.handle_generate_click = function() {
		...
		var begin = $('#modCentralMaps_datepicker_begin').val();
		var end = $('#modCentralMaps_datepicker_end').val();
